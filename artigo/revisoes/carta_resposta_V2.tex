...
We have addressed \textbf{every comment} raised by both reviewers. The revisions include:
\begin{itemize}[nosep]
  \item Implementation of RFC~3161 Trusted Timestamping for external hash anchoring (code, SQL migration, and server integration);
  \item Replacement of ``immutable'' with ``tamper-evident'' throughout;
  \item Addition of formal statistical tests (95\% CIs, Mann--Whitney $U$, Fisher's exact test, effect sizes) with appropriately hedged language;
  \item Explicit repositioning of the contribution as \emph{integration engineering};
  \item Complete rewrite of the comparison table with multi-level scoring and independent methodology;
  \item Expanded three-class threat model with residual-risk analysis;
  \item LGPD compliance mapping table with disclaimer;
  \item Load testing results (autocannon benchmarks at 100/500 concurrent connections);
  \item Structured Limitations section with seven identified threat areas;
  \item Zenodo replication package (DOI placeholder);
  \item Significant text condensation.
\end{itemize}

\noindent Below we provide a detailed point-by-point response. Reviewer comments are highlighted in blue boxes; our responses appear in green; manuscript changes are indicated in brown italics.
...
\begin{itemize}[nosep]
  ...
  \item Fisher's exact test for dispute rates: 0/60 vs.\ 14/292, $p=0.096$ (one-tailed), explicitly noted as \emph{not reaching conventional significance};
...
\change{Section~5 (Impact) --- completely rewritten with statistical tests and hedged language. Section~6 (Limitations) --- new ``Study design'', ``Baseline data quality'', and ``Confounding factors'' subsections.}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Privileged administrator} (root DB access) --- mitigated by RFC~3161 external hash anchoring.
\end{enumerate}
...
\begin{enumerate}[nosep]
  \item \textbf{Positive evidence:} Section~3 now states that the implementation was checked against the OWASP Top~10 checklist and that \texttt{npm audit} reports zero high-severity known vulnerabilities in production dependencies. We also describe current key management practices (JWT secrets and HMAC keys stored as environment variables, never hard-coded).
  \item \textbf{Honest limitation:} Section~3 and Section~6 (``Security'' subsection) explicitly state: ``\textit{No formal penetration test or external security audit has been conducted to date; this represents planned future work.}'' and ``\textit{These do not constitute a comprehensive security assessment.}''
\end{enumerate}
...
\response{We have now conducted and reported load testing. A new load test script (\texttt{scripts/load-test.js}) using autocannon (HTTP benchmarking tool for Node.js) was created with three scenarios: GET / (landing page), GET /api/verify/:protocol (integrity verification), and POST /api/inscricao (submission creation with SHA-256 hashing and async RFC~3161 timestamping). Tests were run at 100 and 500 concurrent connections for 30~seconds each on the same AWS EC2 t3.medium instance.}

A new Section~4.1 (Load Testing) and Table~(tab:loadtest) report the results:
\begin{itemize}[nosep]
  \item Read operations sustain $>$1,400 req/s even at 500 concurrent connections;
  \item Write operations (with hashing and DB insertion) show expected throughput reduction under high concurrency but remain functional;
...
\change{New Section~4.1 (Load Testing), new Table~(tab:loadtest). New file: \texttt{scripts/load-test.js}. Section~6 (Limitations) --- ``Scalability'' subsection.}
...
\begin{quote}
``\textit{Anonymized deployment metrics, statistical analysis scripts (Python), and the load-testing framework are available in the replication package at \url{https://doi.org/10.5281/zenodo.XXXXXXX}.}''
\end{quote}

The Zenodo DOI placeholder (XXXXXXX) will be replaced with the actual DOI upon upload of the replication package before camera-ready submission. The package will include: anonymized metrics (no PII), Python statistical analysis scripts reproducing all reported CIs and tests, the autocannon load-test script, and configuration examples.

\change{Data Availability Statement updated. Section~6 (Limitations) --- ``Reproducibility'' subsection references Zenodo. Section~7 --- replication package mentioned.}
...
\hline
R2.7 & No stress testing & Load testing script + results (100/500 concurrent, 3 scenarios) \\
\hline
R2.8 & LGPD not demonstrated & LGPD mapping table, ``LGPD-aware'' terminology, RIPD disclaimer \\
\hline
R2.9 & No replication package & Zenodo DOI placeholder; replication package prepared \\
\hline
