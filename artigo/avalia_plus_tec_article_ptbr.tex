...
Para avaliar a escalabilidade além da escala de implantação inicial, foram conduzidos testes de estresse usando autocannon (ferramenta de benchmarking HTTP para Node.js) na mesma instância AWS EC2 t3.medium. A Tabela~\ref{tab:loadtest} reporta os resultados para três níveis de concorrência.
...
\hline
\textbf{Cenário} & \textbf{Conc.} & \textbf{Req/s} & \textbf{$p_{50}$ (ms)} & \textbf{$p_{95}$ (ms)} & \textbf{$p_{99}$ (ms)} \\
\hline
...
\hline
\end{tabular}
\caption{Resultados de testes de carga na instância AWS EC2 t3.medium (2 vCPUs, 4~GB RAM). Cada cenário foi executado por 30~segundos usando autocannon. Conc.\ = conexões simultâneas.}
\label{tab:loadtest}
\end{table}
...
\textbf{Fatores de confusão.} O delineamento pré-pós não pode controlar completamente variáveis de confusão como mudanças na composição da equipe administrativa, efeitos de aprendizagem dos avaliadores, diferenças na qualidade das submissões entre coortes e o efeito Hawthorne (avaliadores cientes do monitoramento).

\textbf{Segurança.} Conforme discutido na Seção~3, o modelo de ameaças agora aborda insiders privilegiados via ancoragem externa RFC~3161. Contudo, nenhum teste formal de penetração ou auditoria de segurança externa foi conduzido. As práticas de gerenciamento de chaves (segredos JWT e chaves HMAC armazenados como variáveis de ambiente, não codificados no código) seguem recomendações padrão, mas não foram verificadas independentemente. A implementação foi verificada contra o OWASP Top~10 \citep{Owasp2023} e \texttt{npm audit} reporta zero vulnerabilidades de alta severidade, mas esses não constituem uma avaliação de segurança abrangente.
...
\textbf{Luiz Diego Vidal Santos:} Conceituação, Metodologia, Software, Validação, Análise formal, Investigação, Curação de dados, Escrita -- rascunho original, Escrita -- revisão e edição, Visualização, Administração do projeto. \textbf{Catuxe Varjão de Santana Oliveira:} Conceituação, Metodologia, Validação, Análise formal, Escrita -- revisão e edição, Supervisão.
